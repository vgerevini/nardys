...
public partial class Manager_Modulos_Categorias_Listar : System.Web.UI.Page
{
    #region Variáveis
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            {
                ((Manager_Interna)Master).ExibirMensagem(Resources.Textos.Modulo_Categoria_Mensagem_Salva_Sucesso);
            }
            else if ((Request.QueryString["Sucesso"] == "2"))
            {
                ((Manager_Interna)Master).ExibirMensagem(Resources.Textos.Modulo_Categoria_Mensagem_Atualizado_Sucesso);
            }
    ...
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<Categoria>)
        {
            IEnumerable<Categoria> dados = (IEnumerable<Categoria>)listPager.DataSource;
