...
public partial class Manager_Modulos_Usuarios_Listar : System.Web.UI.Page
{
    ...
    protected void grdDados_SortCommand(object source, DataGridSortCommandEventArgs e)
    {
        if (listPager.DataSource != null && listPager.DataSource is IEnumerable<Usuario>)
        {
            ...
            switch (e.SortExpression)
            {
                ...
                case "Van":
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.NomeFantasia).ToArray();
                    else
                        dados = dados.OrderByDescending(u => u.NomeFantasia).ToArray();
                    break;
                default:
                    if (AscendingSort)
                        dados = dados.OrderBy(u => u.Nome).ToArray();
