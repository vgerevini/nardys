  ...
  $.fn.checkedPolyfill = function (options) {
    ...
    return this.bdneach(function () {
      ...
      if ($self.attr('type') === 'radio') {
        $('input[name="' + $self.attr('name') + '"]').click(function() {
          checkValue($self);
        });
...
(function ($, doc) {
  ...
  if (!HAS_NATIVE_SUPPORT) {
    $.fn.val = function () {
      ...
      // Handle .val()
      if (!args.length) {
        placeholderTxt = $(el).attr('placeholder') || $(el).attr('title') ;
  ...
  $.fn[PLUGIN_NAME] = function (options) {
    ...
    var settings = $.bExt(DEFAULTS, options);
    ...
    return this.bdneach(function (i, el) {
      ...
      if (!HAS_NATIVE_SUPPORT) {
        ...
        setPlaceholder = function () {
          ...
          if (!$el.val()) {
            if (isPassword) {
              ...
              $el.val(placeholderTxt).addClass(settings.cssClass).data('placeholderEnhanced',true);
            }
        ...
        if (!isPassword) {
          ...
          removePlaceholder = function () {
            if ($el.data('placeholderEnhanced')) {
              $el.data('placeholderEnhanced',false)
              el.value = '';
              $el.removeClass(settings.cssClass);
            }
          };
        ...
        // And for password inputs
          ...
          fakePassw = $('<input>', $.bExt(copyAttrs(el), {
              'type': 'text',
      ...
      // Normalize placeholder behaviour and style in modern browsers if normalize mode is on
        ...
        el._placeholder = $el.attr('placeholder') || $el.attr('title') ;
